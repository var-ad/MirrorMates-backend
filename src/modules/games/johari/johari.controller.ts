...
export const listAdjectivesController = asyncHandler(
...
export const listMySessionsController = asyncHandler(
...
export const selfSelectController = asyncHandler(
  async (req: Request, res: Response) => {
    const result = await saveSelfSelections(
      req.params.id,
      req.user!.id,
      req.body.adjectiveIds,
    );
    res.json({ message: "Self selections saved", ...result });
  },
);

export const updateInviteSettingsController = asyncHandler(
  async (req: Request, res: Response) => {
    const session = await updateInviteSettings(
      req.params.id,
      req.user!.id,
      req.body,
    );
...
export const inviteMetaController = asyncHandler(
  async (req: Request, res: Response) => {
...
export const inviteSubmitController = asyncHandler(
  async (req: Request, res: Response) => {
    const result = await submitInviteFeedback({
      inviteToken: req.params.token,
      displayName: req.body.displayName,
      adjectiveIds: req.body.adjectiveIds,
      fingerprint: createFingerprint(req, req.params.token, req.body.peerId),
    });
    res.status(201).json(result);
  },
);

export const sessionResultsController = asyncHandler(
  async (req: Request, res: Response) => {
...
export const latestReportController = asyncHandler(
  async (req: Request, res: Response) => {
